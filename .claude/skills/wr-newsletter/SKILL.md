...
1. Record the post's hash (`shasum -a 256 <post-path>`, kept in context, NOT written to disk; ADR-046 declined a marker file). Then remediate each finding minimally in the post text (lead with any promise/payoff gap where the post names stories it does not resolve, and any causal overstatement).
