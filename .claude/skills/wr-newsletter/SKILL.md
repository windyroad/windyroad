...
**Skip-on-upstream-REJECTED.** Skip step 15.25 entirely (do not invoke the editor) if and only if the terminal verdict from the critic loop at step 15 (newsletter-critic per ADR 033) is `VERDICT: REJECTED` (round-3 exhausted). On `VERDICT: PASS` or `VERDICT: PASS_WITH_AUTHOR_OVERRIDES` (the two terminal publish-ready verdicts; `PASS_WITH_AUTHOR_OVERRIDES` is publish-ready per ADR 025 and the editor runs against it as it would against a `PASS`), run the editor as normal. `WEAKNESSES_FOUND` never reaches this gate: it triggers a critic re-invoke at step 15, so by the time control reaches step 15.25 the verdict is always one of the three terminal values.
...
3. Re-invoke the skeptic once against the revised post. **Skip this re-invocation (ADR-046) when the artefact is byte-identical to the version the findings were collected against**, compared by re-running `shasum -a 256` on the artefact and matching the digest recorded at collect. Deterministic contributors are re-run regardless; only agent invocations are skipped. When the hash is unavailable, re-run. A skipped round is still consumed and is recorded as declined in the reviews sibling. Anything still standing is a recorded residual advisory, tagged `RESIDUAL (round 2, accepted)` under `## Skeptic Review (LinkedIn post)` in the reviews sibling, and surfaced in the Tom-summary. No second round, no author-override arm.
4. A post-body edit still re-enters the gate set per section 15.6, and per ADR-043 condition (c) the counter does not reset: if the LinkedIn voice gate at 15.5 then forces a further edit, the skeptic gets one more look and remaining findings go straight to residual advisory. A look declined under ADR-046 because the artefact was unchanged still consumes that counter.
...
3. Anything still standing is a recorded residual advisory tagged `RESIDUAL (round 2, accepted)` under `## Shape Review (LinkedIn post)` in the reviews sibling. No second round, no author-override arm.
4. A post-body edit re-enters the gate set per section 15.6, and **per ADR-043 condition (c) the counter does not reset**: if the LinkedIn voice gate at 15.5 or the skeptic then forces a further edit, each gets one more look and remaining findings go straight to residual advisory. The cut-to-fit warning matters most here: if a remediation pushes the post back to the platform limit, that is the finding re-firing, not a new one. A look declined under ADR-046 because the artefact was unchanged still consumes that counter.
