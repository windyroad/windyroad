...
- **Reddit r/MachineLearning** (Playwright cache primary; no RSS fallback). Same rung structure as r/LocalLLaMA.
  ...
  1. **Playwright cache** at `.cache/newsletters/reddit-machinelearning/<YYYY-MM-DD>.json` (P014b fetcher; ADR-029). Cache is populated out-of-band by `npm run fetch:newsroom -- --source=reddit-ml`. Filter the items downstream: focus on research results and industry developments, skip pure academic discussion.
  2. **No fallback fetch**.
