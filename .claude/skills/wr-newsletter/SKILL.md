...
**Known source gap (P014)**: any `reddit.com` URL is refused by the WebFetch tool layer before the network call is made. Both Reddit entries below default to `source_failures` every run until the Playwright helper from P014 lands and writes a local cache that step 2 reads via `Read`. The entries are kept here (not deleted) so the canonical config is in place for the helper to consume once shipped.
