...
**Known source gap (P014b)**: any `reddit.com` URL is refused by the WebFetch tool layer before the network call is made. Both Reddit entries below default to `source_failures` every run until the Playwright helper from P014 is extended to Reddit (P014a shipped the walking skeleton for OpenAI; P014b adds Reddit coverage with subreddit listing parsing and the consent-gate handling). The entries are kept here (not deleted) so the canonical config is in place for P014b to consume; the cache-read precedence will mirror the OpenAI block above (Playwright cache fresh, then continue-on-fail per tier-2 policy).

- **Reddit r/LocalLLaMA**: `WebFetch https://www.reddit.com/r/LocalLLaMA/top/.json?t=week` with prompt: "Extract the top 10 posts of the week. For each: title, permalink URL, one-sentence summary. Skip memes and low-signal discussions." Currently expected to fail with a tool-layer refusal; record in `source_failures` and continue.
- **Reddit r/MachineLearning**: `WebFetch https://www.reddit.com/r/MachineLearning/top/.json?t=week` with prompt: "Extract the top 10 posts of the week. For each: title, permalink URL, one-sentence summary. Focus on research results and industry developments, skip pure academic discussion." Currently expected to fail with a tool-layer refusal; record in `source_failures` and continue.
