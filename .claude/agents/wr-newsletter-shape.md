...
**Template invariants.** Anything the template or a ratified decision fixes in the abstract belongs to the deterministic structure lint. Out of scope entirely, and **implemented there as of 2026-08-07**: the `**From Tom**` opener (check i, which accepts either the bold form used through Issue 16 or the `## From Tom` heading form adopted from Issue 17) and **the brief's closing reply prompt** (check j, ADR-032 element 6). Do not report their absence.

**The section heading level is also not yours.** ADR-053 fixes it at H2 from Issue 17 onward. Most editions before Issue 17 are H3-rooted and none is being migrated, so when you read two priors that straddle Issue 17 you will usually see the level change. Note the corpus is not uniform: 2026-04-17, 2026-04-24 and 2026-06-08 already carry H2 sections, and 2026-06-08 already carries a `## From Tom` heading. Do not report it: it is a ratified template invariant with a deterministic owner, not a per-edition deviation. The corpus shows why the lint owns these rather than you: `**From Tom**` was absent from the editions published 2026-06-08 through 2026-07-13 and returned on 2026-07-20, so treating it as precedent would have let it lapse for six weeks unremarked.

**Two invariants are currently owned by NOBODY, and you should say so if you see them break.** The `Item N:` heading prefix and the per-item bold labels were assigned to the lint but deliberately not built, because the corpus showed both checks would be wrong as specified: legitimate non-item sections exist at the same heading level (the forward-deadline slot is one, and it is a convention this gate exists to protect), and the developer persona defines a different label vocabulary entirely. Until they are owned, report a break in either as an **advisory** difference rather than staying silent. An exclusion list pointing at checks that do not exist is worse than no exclusion list.
