...
- If any weaknesses surfaced and this is round 3:
  - If **every** remaining weakness was named by the drafter in `prior_weaknesses` as an editorial-judgement override (a defensible editorial reason to keep the artifact as-is despite the critic call): `VERDICT: PASS_WITH_AUTHOR_OVERRIDES`. **Read the ADR-052 note at the end of this file before you emit that verdict: it no longer clears publication, and the drafter's own say-so is not what clears it.** The verdict block names each overridden weakness verbatim via the `OVERRIDDEN:` lines (see Step 6). Author overrides apply only at round 3; rounds 1 and 2 continue to emit `WEAKNESSES_FOUND` regardless, so the drafter still gets a chance to resolve the weakness before round 3 exhaustion. See ADR 025 (verdict semantics preserved) + ADR 035 (override mechanism is prompt-level editorial framing, not a structured check-ID list).
...
**`PASS_WITH_AUTHOR_OVERRIDES` is not a pass (ADR-052, ratified 2026-08-10).** It used to be described here as a publish-ready signal. It is not one any more. Every reviewer gate now blocks publication, and a gate passes only when it holds no finding it classified as a defect. `PASS_WITH_AUTHOR_OVERRIDES` says the opposite: findings remain, and the reason they are tolerated is that the drafter said so in `prior_weaknesses`. The drafter wrote the passage, so that is self-certification, which is exactly the deferral path ADR-052 closes and the confirmation bias ADR-043's no-override-arm already refuses elsewhere in this pipeline.

What to do instead at round 3. If the remaining weaknesses are real defects, return `REJECTED` and say why. If they are editorial judgement rather than defects, say that in your prose and mark each one a preference, so the skill records it as a finding for Tom to clear on the record with his own stated reason. Either way the decision belongs to Tom and is recorded as his, not inferred from the drafter's framing. You may still emit `PASS_WITH_AUTHOR_OVERRIDES` where the vocabulary requires it, but treat it as "weaknesses remain, unresolved" rather than as clearance, and do not let it be the reason an edition ships.
