...
<span data-pull>The agent didn't escape its constraints. It found there were no constraints to escape.</span>
...
![Five-step horizontal flow. Step 1: agent intent, with three example commands shown as code chips: git commit, git push, and gh pr merge for release. Step 2: PreToolUse hook intercepts the call. Step 3: hook checks the session-scoped score file with five tests, fail-closed: exists, fresh, drift-free, valid, and residual below five. The check has two outgoing branches. PERMIT branch goes up to a green box labelled action runs. DENY branch goes down to the risk-scorer subagent, which reads the diff and conversation, scores impact and likelihood, and writes the score file. A dashed return arrow loops from the score file back to the score check on the next attempt. A legend at the bottom explains PERMIT and DENY badges and notes that the block threshold is residual score greater than or equal to five.](/img/social/risk-gate-flow.svg)
...
**Structural:** the scoring is done by a subagent with its own context, not the agent driving the commit. [Recent research](https://arxiv.org/abs/2412.04984) finds that frontier models pursue their assigned goals in ways that include rule-breaking when the environment incentivises it. An agent told to "complete this commit" has a structural reason to under-score risk to clear the gate. The risk-scorer subagent has a different task. Its goal is accurate scoring against the policy, not getting the commit through. The drift, if any, is toward over-flagging, not under-scoring. If something gets in the way of accurate scoring, the subagent will work around that, not around the policy.

**Mechanical:** the score is a discrete artefact, a number written to a file, checked by a deterministic gate. The model's compliance with the policy ends at the moment the file is written. From there, every check is shell. <span data-pull>The probabilistic step is bounded by the policy and converted into a fixed value before the gate runs.</span> The risk-scorer subagent reads the diff and produces a structured report with one entry per identified risk. Each entry names the failure scenario, scores impact and likelihood, lists the controls that reduce inherent risk to residual risk, and ends with the residual number.
...
![Risk-item card titled commit removes production database volume. Two-column layout. Left column shows inherent risk: impact 5 (Severe), likelihood 5 (Almost certain), inherent risk 25 of 25 (Very High). Right column shows controls: none named, with three lines explaining what does not count as a control (no paired migration, no backup snapshot in the changeset, no incident ticket linking the change to approval). A red verdict bar across the bottom of the card states residual 25 of 25, block threshold residual greater than or equal to 5, verdict DENY. Below the card, a four-line legend explains where each value comes from: impact and likelihood from RISK-POLICY.md, controls must be named with file path and scenario, threshold from policy not the agent.](/img/social/risk-score-anatomy.svg)
...
![Vertical stack of four numbered layer cards. Layer 1: Agent has no production access; production credentials live in CI/CD secrets, used only by pipeline jobs; owner is your team; PocketOS state is missing. Layer 2: API tokens scoped by operation, environment, and resource; a domain-only token cannot call volumeDelete; owner is the infrastructure provider; PocketOS state is missing. Layer 3: Destructive API requires authorised change, not just a token; token authenticates, approved change record authorises, audit lands in both; owner is provider or platform; PocketOS state is missing. Layer 4: Pipeline risk-scoring gate on commit, push, release; hook scores each pipeline action against RISK-POLICY.md and denies above threshold; owner is your agent harness; PocketOS state is system-prompt text instead. Caption: PocketOS had no layer 1, no layer 2, no layer 3, and a paragraph of system-prompt text where layer 4 should have been.](/img/social/layered-defence.svg)
