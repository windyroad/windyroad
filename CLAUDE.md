...
Before propagating a ticket's `Fix Strategy` upstream-placement claim ("fix is in upstream X plugin", "this is a cross-package job in repo Y") into downstream actions (WSJF re-rates, upstream-blocked classifications, skip decisions), treat the proposal as a hypothesis. Verify two things: does the proposed home's domain match the work's domain? Is this project in a position to place work there? Recommending placement to an external repo is at most a proposal the maintainers can reject. Parroting a placement claim back as load-bearing fact misclassifies tickets and burns iteration budget. See `~/.claude/projects/-Users-tomhoward-Projects-windyroad/memory/feedback_verify_upstream_placement_before_propagating.md` (P045).

Before asserting a **negative or absence claim** about project state ("X does not do Y", "the repo has no Z", "this isn't wired up"), treat a bounded search that found nothing as inconclusive, not as proof. "Not in the files I searched" is not "does not exist": the mechanism may be human-out-of-band (a secret bridged into a store by hand, a step performed in another tool, a manual deploy) that no repo grep can reveal. This also applies to negative results from predicates and gates (an empty grep, a `no-X-found` directive): verify the result's premise before acting on it. A negative claim requires either exhaustive verification or asking the user *before* asserting. See `~/.claude/projects/-Users-tomhoward-Projects-windyroad/memory/feedback_verify_project_state_before_writing.md` (P103, recurrence of P032).

Before propagating any artefact reference cited in a **governance-subagent verdict** (architect, jtbd, risk-scorer, sw-critic, and the rest), such as an `ADR-NNN`, `RFC-NNN`, `JTBD-NNN`, a skill name, a script or tool name, or a file path, into a downstream action (invoking the named tool, flagging it as a load-bearing precondition, creating tickets / commits / ADR cross-references that name it), verify the cited artefact actually exists on disk first (`ls` / `grep` / `find`). Subagent output is hypothesis until verified, exactly like ticket prose (P032) and Fix-Strategy placement claims (P045). The asymmetric trust model holds: the subagent need not self-verify; the consumer (the orchestrator) verifies before propagating. Evidence: on 2026-06-02 an architect verdict cited `ADR-077` and a `wr-architect-generate-decisions-compendium` tool as a load-bearing precondition, both absent from disk, caught only by applying P032 discipline to the verdict. This is the third surface in the verify-before-X family, after ticket prose (P032/P103) and Fix-Strategy placement (P045). See `~/.claude/projects/-Users-tomhoward-Projects-windyroad/memory/feedback_verify_subagent_references_before_propagating.md` (P082).
