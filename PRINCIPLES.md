...
- Deciding the correct fix across code + tests + docs
- Choosing the next investigative step when failure output is incomplete

**Rule of thumb**: execute and record deterministic steps (commands, outputs, gates). When a deterministic step fails, provide the captured failure output plus relevant context to the LLM to diagnose and propose a fix. After the fix, rerun the deterministic step and repeat until success or a clear non-recoverable condition.
...
Use separate LLM sessions for generation and review where practical:

1. **Generator LLM**: Produces output based on requirements and context
2. **Review LLM**: Separate session evaluates output against requirements and validation criteria — it does NOT modify output, only reviews
...
This principle recognizes that AI-assisted work requires different validation strategies than traditional code — we guide through prompts and schemas, validate through independent review, not post-hoc procedural verification.
