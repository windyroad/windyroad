...
review, an outcome-blind activation decision, a pre-submission content freeze,
...
non-activation package before the pre-submission content freeze.
...
runner, two-phase freeze records, and analysis code. Current unfrozen hashes are:
...
The first freeze phase records the study, exact content commit, selected branch,
release-safe registration-bundle hash, queues, systems, artifact hashes, and
runtime-critical file hashes before OSF
submission, while explicitly leaving outcome calls unauthorised. After
registration and download verification, a separate execution record binds the
OSF identifier and timestamps to the raw-byte freeze-record hash, content commit,
branch, bundle hash, and exact queues. The runner recomputes these bindings and
the actual bundle, runtime files, and five artifact hashes before creating output
or starting client preflight. Both detached records are excluded from the bundle
