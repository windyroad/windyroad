...
for H1--H3. A considered Ollama Cloud replication was not activated and
contributes no observation or analysis to this registration.
...
recognise a composition before its activation boundary. A considered Ollama
Cloud replication was not activated; any future test requires a separate
preregistration and cannot alter these confirmatory results.
...
shuffled with a seeded Fisher--Yates algorithm. No outcome-dependent stopping,
...
\begin{table}[ht]
\centering
\caption{Confirmatory review systems.}
\label{tab:systems}
\ifdefined\EnableTaggedPDF\tagpdfsetup{table/header-rows={1}}\fi
\begin{tabular}{@{}lll@{}}
\toprule
...
Ollama Cloud was considered as a separate exploratory replication. The dated
terms assessment and provider correspondence remain audit history, but the
frozen decision is \code{not-activated}. This registration authorises no Ollama
purchase, preflight, implementation, queue, dispatch, observation, or analysis.
Any future Ollama experiment requires a separate preregistration.
...
No Ollama observation or analysis is part of this registration. Confirmatory
analysis rejects Ollama, missing, extra, or mixed rows and also rejects a ledger
unless every frozen authorized call has exactly one terminal result.

The products and sample are a convenience selection determined by subscriptions
available to the author, not an 80\% power target. An earlier 20,000-replication single-system
...
ledger, or output artifact has been implemented or generated, and the frozen
decision excludes Ollama from this study. An earlier
...
require exact-commit revalidation before registration. Ollama's AI support agent
did not grant or deny permission and offered human escalation; that exchange is
retained as audit history and cannot activate Ollama within this frozen study.
...
families, two confirmatory products, and compact
...
Public research data are limited to sanitized derived
...
archived. A later outcome-blind worktree review also retained a \code{do not
...
corrections were incorporated before this reviewer-input candidate was frozen.
Later outcome-blind worktree reviews found no blocker, but they are not
phase-one approval. All three review roles must approve the same exact frozen
commit before the detached phase-one attestation and OSF submission. The marker
cannot itself represent approval or authorize an outcome call.
...
Frozen reviewer-input hashes are:
...
fixed 14-member release-safe allowlist. It includes the study-local licence and the privacy-preserving
...
confirmatory-only branch also requires the machine-readable Ollama
\code{not-activated} decision and false pre-outcome state flags.
