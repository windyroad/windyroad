...
- **Anyone can propose decisions** - In autonomous mode, voder proposes decisions; in interactive mode, users propose decisions
...
All decisions follow the MADR 4.0 format with voder-specific extensions:
...
When a user works with voder via Copilot or Cursor:
...
5. If decision conflicts with existing decision, voder asks user how to proceed

### Autonomous Creation (Voder Mode)

When voder runs autonomously:
...
4. Team/voder validates successful production use
...
Someone wants to try alternative approach
  ↓
Request deviation (in interactive mode) or voder creates experiment (autonomous)
  ↓
...
5. **Request-based**: Team or voder identifies need for review
...
2. **Business context** that voder may not have
