...
- **The unratified maintainer persona is a hard stop on executable internal-tooling work, not a caveat.** `docs/jtbd/internal-maintainer/` plus JTBD-400/401/402 are all `human-oversight: unconfirmed`. The JTBD edit gate withholds its marker on ISSUES FOUND, and "reasons from an unratified persona" is one an agent cannot clear, so the class is unreachable AFK until `/wr-jtbd:confirm-jobs-and-personas` runs once. Prose still lands; enforcement code does not. Anchor tickets to these jobs in prose under `## Related`, never in header lines, and say the anchoring is provisional. ([what-you-need-to-know.md](./what-you-need-to-know.md), [governance-iteration-friction-2026-08-09-p115-iter.md](./governance-iteration-friction-2026-08-09-p115-iter.md))
