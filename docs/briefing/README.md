...
- **The compendium entry hook writes the entry for you on EVERY write to an ADR, not just when one lands, and it re-derives the whole entry from the body.** So never hand-annotate an entry: annotations are silently replaced on the next touch, as `fea3cec`'s two were on 2026-08-09. Expect `docs/decisions/README.md` in your staged diff after any ADR edit and read it. Two things it does not do and you must: scrub the em-dash in the heading it writes (the no-em-dash hook blocks until you do; this extends P087, which names only the retired full-file generator) and fix the derived counts, now ticketed as P135. ([what-will-surprise-you.md](./what-will-surprise-you.md))
