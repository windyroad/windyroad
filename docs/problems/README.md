...
> Last reviewed: 2026-08-29 **P120 moved back to Verification Pending**: the 2026-08-28 flip-back to Known Error was reversed on Tom's instruction. It rested on Issue 19's twenty-four rounds breaching the ticket's one-round criterion, and the two count different things -- the twenty-four counts rounds over the body, while the criterion and ADR-043 condition (c) cap remediation rounds inside a single body pass, a number the reviews file never states. The backlog now has no Tier 0 row.
> Run `/wr-itil:review-problems` to refresh WSJF rankings.

## WSJF Rankings

Dev-work queue only. Verification Pending and Parked tickets are excluded per ADR-022 and appear in their own sections. Rows render tier-first (Tier 0 Critical-bypass, Tier 1 Inbound-reported, Tier 2 Internal), then within each tier by `(WSJF desc, Known-Error-first, Effort-divisor asc, Reported-date asc, ID asc)` so top-to-bottom order matches `/wr-itil:work-problems` Step 3 selection. No row is Tier 0: P120, which held the only critical bypass on Severity 20, returned to Verification Pending on 2026-08-29 and now sits in the Verification Queue. No ticket carries a security classification or links an incident, and none carries `**Origin**: inbound-reported`, so every row is Tier 2. <!-- REPORTED-FIRST-TIER-SOURCE: /wr-itil:work-problems SKILL.md Step 3 (ADR-076) -->
...
Nine rows. Eight read `no - not observed` and all eight stay: the 2026-08-29 review was an AFK pass, so no fix in those rows was exercised and no new evidence was produced, and under the evidence-first cell there is nothing to close on. The ninth, P120, re-entered the queue on 2026-08-29 when the 2026-08-28 flip-back was reversed, and it is the one row carrying observed evidence, because Issue 19 exercised its fix. The other eight need Tom's out-of-band knowledge and are held for the next interactive review rather than auto-closed.
...
| P120 | Editor and skeptic gates surface findings to Tom instead of remediating them, so their output becomes his review burden | 2026-08-05, repo-local ADR plus skill and agent wiring, no changeset (private root package, nothing shippable changed) | yes - observed: Issue 19 (`src/newsletters/published/leader/2026-08-24/2026-08-24.reviews.md`) ran the ADR-043 loop after every round of both passes with findings acted on in the body, and routed exactly three cross-boundary survivors to Tom, who ruled on all three on 2026-08-24 -- the escalation path working, not the dead end this ticket opened against |
