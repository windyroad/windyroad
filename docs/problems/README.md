...
|------|-----|-------|----------|--------|--------|----------|--------|
| 24.0 | P126 | The deps refresh chain creates a manifest desync and its own recovery path cannot clear it | 12 | Known Error | S | 2026-08-05 | internal |
| 16.0 | P118 | Newsletter publish step uses `git mv` on untracked drafts, which fails | 8 | Known Error | S | 2026-07-27 | internal |
| 16.0 | P125 | Nothing recomputes WSJF on a status transition, so the Open multiplier persists and halves the ticket's rank | 8 | Known Error | S | 2026-08-05 | internal |
...
| P117 | Tighten existing newsletter gate prompts for the lower-frequency external-review classes | 2026-07-15, three prompt/rubric edits | no - observed regression: three editions since the fix; Issue 16 records six further external editorial rounds (`2026-08-03.reviews.md:213-217`). Flip back with `/wr-itil:transition-problem 117 known-error` |
