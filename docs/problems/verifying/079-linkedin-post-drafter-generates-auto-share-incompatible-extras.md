...
- [ ] **Add a promptfoo behavioural eval covering the LinkedIn-post template shape** (durable regression guard). The eval should assert the `/wr-newsletter` step 15.5 / step 16 LinkedIn drafter produces a `.linkedin.md` sibling that is frontmatter (`post-type` + `companion-to`) + post body ONLY, and FAILS if the auto-share-incompatible extras silently return: no `## LinkedIn Post` heading wrapper, no `## Image` section, no `## Notes for posting` section, no manual "Read the full issue" link line in the body. This is the remaining open work that keeps P079 unclosed. **No promptfoo infrastructure exists in this consumer repo yet** (no `eval/`, no `promptfooconfig.*`, promptfoo not a dependency), so standing up the eval harness is a prerequisite and puts this above an S bounded-addition; recorded here for a future iteration rather than implemented this iter (Tom direction 2026-06-17).
...
**Architect follow-up (RESOLVED 2026-06-17)**: ADR-026 body lines 78 and 105 described the old three-heading sibling shape. Amended in place 2026-06-17 (P079): added a dated `## Amendment Note (2026-06-17, P079)` block (mirroring the existing ADR-039 amendment-note pattern), narrowed the Decision-Outcome sibling-shape description, and updated confirmation criterion 2 to the minimal frontmatter + body shape. `docs/decisions/README.md` ADR-026 compendium entry refreshed to match (architect + JTBD gates PASS). Note: the deprecated `wr-architect-generate-decisions-compendium` full-regen reintroduces pre-P025 em-dashes from quoted ADR bodies, so the entry was refreshed by hand in the existing generator format rather than via the full-regen script.
