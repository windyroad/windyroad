...
>
> 1. **ADR-043 is ratified.** `human-oversight: confirmed`, `oversight-date: 2026-08-05`. Every "ADR-043 is unratified" statement below, and the deferral logic built on it, is discharged. This ticket is unblocked.
...
2. **New, from the architect review 2026-08-07: coordinate with P121 rather than landing independently.** Both tickets amend ADR-043's step-15.37 contract within days of each other. Land them as **one** `## Amendment <date> (P121, P122)` section covering both, not two sequential patches on the same section. Two things must be settled jointly:
   ...
   **One step-15.37 change of this ticket's is NOT covered**: Fix Strategy section 5's skip-the-paired-re-invocation rule (when every collected finding is stop-and-surface, skip the re-invocation and record the residuals directly). It appears in none of the five clauses, and its original warrant has been withdrawn by the ceiling correction. ADR-043 confirmation criterion 3 pins step 15.37's contents verbatim, so shipping it without an amendment clause would be a live confirmation violation against a ratified decision. Re-argue the rule on cost merits, then add it as a sixth clause to the existing 2026-08-07 section rather than opening a second dated section on this file.
