...
- Recurrence 2026-08-25: still reproduces. The `wr-itil-check-upstream-responses` shim on PATH resolves to the 1.1.1 cached build (1.1.2 and 1.2.0 are also cached), and its script body still emits U+2014 in the audit-log heading. Normalized to the colon form before the ADR-014 commit. Upstream issue 319 still OPEN with zero comments as of the 2026-08-25 outbound-responses poll, so this is now the fourth recorded occurrence with no maintainer response.
- Recurrence 2026-08-28: still reproduces on the wr-itil 2.1.0 skill. The `wr-itil-check-upstream-responses` shim on PATH still resolves to the 1.1.1 cached build (1.1.2, 1.2.0 and 2.1.0 are also cached), and the U+2014 heading is present in the 2.1.0 script body too (line 361 of `check-upstream-responses.sh`, identical across all four cached versions), so a shim-resolution upgrade would not fix it. Normalized to the colon form before the ADR-014 commit. Upstream issue 319 still OPEN with zero comments as of the 2026-08-28 outbound-responses poll, so this is the fifth recorded occurrence and roughly two months with no maintainer response.

**Anchoring: JTBD-402 (Land the fix where the defect lives), Internal Maintainer persona.** The header lines carried at capture, `**JTBD**: JTBD-007` and `**Persona**: plugin-developer`, have been removed. Both were upstream `agent-plugins` values; neither resolves here. There is no local JTBD-007 and `plugin-developer` is not one of this repo's five personas, so unlike its siblings this pair was dangling rather than misresolving. `docs/jtbd/internal-maintainer/` models the person who pays for it. JTBD-402 is the right job rather than JTBD-400 because `/wr-itil:check-upstream-responses` is one of that job's named screens and the defect is on the outbound path itself: it is the job's third outcome, that the path works rather than charging a manual strip on every pass. Two months of recurrences on a still-open upstream issue, filed 2026-07-03 and reproduced twice since, is also the job's first outcome under strain: a wait that is meant to be a staging state and has not moved. Both the persona and JTBD-402 carry `human-oversight: confirmed`, `oversight-date: 2026-08-09`, so this anchoring is settled rather than provisional (the caveat this ticket carried predated that ratification). Recorded in prose rather than `**JTBD**` / `**Persona**` header lines, per local convention.
