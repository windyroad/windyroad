...
## Observed 2026-08-26: an orphan did unsupervised work, and two mitigations failed

The ticket's Impact-4 reasoning was that an orphan *can* commit with every verification surface detached. On the third interrupted loop of the same session it *did* work, and the reasoning stops being hypothetical.

An iter dispatched against P128 was interrupted. The subprocess survived, ran on unsupervised, created a new problem ticket, refreshed two README files, and picked up a third file regenerated by a compendium hook. None of it was committed, so it sat in the working tree with no orchestrator to classify it. The work was salvaged by hand from the orchestrator main turn, with the commit gate fired fresh on the live session rather than on the dead subprocess's markers. Its quality was good, which is beside the point: good work and supervised work are different properties, and only the second is what the loop's design guarantees.

**Two mitigations were tried in the same session and both failed. Recorded because each looks correct and is not.**

*Recording the PID from the dispatch does not give you the subprocess.* The wrapper was changed to capture `$!` immediately after backgrounding `claude -p` and write it to a file. On the next interruption the recorded PID was already dead while `pgrep` still found a live `claude -p`, at a different PID with a different parent. The `claude` entrypoint does not stay at the PID the shell backgrounds; whatever the shell captures is not the process that holds the repository. A PID file written this way gives false confidence: it reports a clean shutdown while the real process runs on.

*`pgrep -f 'claude -p' | head -1` finds one orphan and hides the rest.* An earlier cleanup used exactly that, killed a 28-minute orphan, re-checked, and reported zero live subprocesses. A later check found a 30-minute orphan that had been running throughout, so it predates the cleanup that declared the tree clear. Either the `head -1` took the other match, or the recheck sampled during the first process's teardown. The re-check reporting zero is what makes this worth recording: the verification agreed with the incomplete cleanup.

Both failures are the shape this repository has now recorded seven times: a check reporting a condition it did not establish. The PID file reports a shutdown it never confirmed. The `head -1` cleanup reports completeness it never had. Any fix here needs to kill by enumeration and verify by re-enumeration, not by sampling once.
...
- [ ] Create a reproduction: dispatch an iter, kill the wrapper, assert no `claude -p` survives. Enumerate ALL matches rather than `head -1`, and re-enumerate after a settle delay, because both shortcuts produced false clears on 2026-08-26.
- [ ] Establish what PID the `claude` entrypoint actually runs at, since the shell's `$!` is demonstrably not it.
