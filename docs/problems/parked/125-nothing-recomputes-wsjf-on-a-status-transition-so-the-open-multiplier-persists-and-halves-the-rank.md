...
**Persona**: anchoring left explicitly unconfirmed. The prior `developer` value named this repo's reader persona (a working engineer who reads Tokens Spent on LinkedIn), which has no relationship to the problem backlog. Update 2026-08-09: the maintainer persona these two lines say is unmodelled now exists, written 2026-08-08 as `docs/jtbd/internal-maintainer/`, and this ticket anchors to JTBD-400 (Trust what the loop did while I was away), whose fifth outcome is a surface whose output must be re-derived to be believed. Left as header-line prose rather than converted to the `## Related` shape the sibling tickets use, because the ticket is Parked and excluded from ranking; convert it at un-park. The persona and job are `human-oversight: unconfirmed`, so the anchoring is provisional.
