...
> **Amended 2026-08-07, extended 2026-08-09.** Two figures in this record are now point-in-time. The commit threshold moved to **5 or below** (`Threshold: 5`) on 2026-08-07, which makes the `< 5` figures in the Decision Outcome and the Consequences stale, and makes the `>= 5` figures in the Confirmation stale in the other direction: blocking now starts at 6, not at 5. The label bands moved on 2026-08-09 to `3-5 Low / 6-9 Medium`, which makes the Labels table below stale in its Low and Medium rows; the Very Low, High and Very High rows are unchanged, as are the product matrix and the formula this record decides. `RISK-POLICY.md` is the enforcement surface and the single source of truth for both moved values; where it and this record disagree, the policy wins. Retained rather than corrected because this record is the reasoning as it stood at the time. See ADR-049 and RISK-POLICY.md Amendment History.
