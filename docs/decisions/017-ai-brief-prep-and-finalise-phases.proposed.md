...
- Finalise re-fetches tier-1 sources only (Anthropic; OpenAI via Google News RSS workaround per P010; DeepMind; US/UK AI-policy news outlets per P051: Politico, Axios, Reuters via a single combined Google News RSS scoped query). Tier-2 and tier-3 are carried forward from the prep run. New tier-1 stories that land between prep and finalise are surfaced via per-item interactive capture (Agree / Adjust / Drop) before the final critic pass.
