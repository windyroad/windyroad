...
**Most, not all, and the exceptions matter, though they matter less than the first draft of this record claimed.** A count of the published corpus on 2026-08-10 found three editions that break the pattern. 2026-04-17 carries four H2 headings alongside five H3s and 2026-04-24 carries five alongside six. Those two are the weaker evidence: their H2 headings are Voice Review, Content Risk Review, Map Delta and similar review meta-content that ADR-026 later moved into sibling files, so they are mixed in bytes rather than in anything a reader saw. The one that counts is 2026-06-08, which carries `## From Tom` and `## Also worth noting` as H2 headings in the reader-facing outline. Its five items are still `### Item N`, so it is a partial precedent and not the shape this decision adopts, which is the opener heading and H2 items together. What it does establish is that a From Tom heading and H2 sections shipped, were read, and drew no complaint.

This is worth stating because the argument used to refuse the accessibility finding, twice, was that sixteen editions of precedent stood against it. That argument was wrong twice over. It was not the real blocker, which was the lint. And the precedent it invoked does not exist in the uniform form it was claimed in: the corpus already contains H2 section headings and already contains a From Tom heading, in the same edition. Nobody checked. The claim was asserted from an impression of the corpus rather than from a count of it, which is the same failure this project has recorded before as asserting project state without reading the files.
...
The corpus count makes the choice a little easier than it looked, and only a little. Issue 17 is not introducing every part of this shape from nothing: 2026-06-08 already shipped the opener heading and H2 section headings, which is evidence that both render acceptably on the channel that matters.
...
**Correction after ratification, 2026-08-10.** Tom ratified this record and then delegated the disposition of a defect found in it immediately afterwards. The Context section originally said 2026-06-08 carries "exactly the shape this decision adopts". That is false: the edition has the opener heading and two H2 sections, but its five items are H3, so it is a partial precedent. The two earlier mixed editions were also presented as stronger evidence than they are, since their H2 headings are review meta-content rather than anything a reader met. The three passages above are corrected.

The decision is unchanged, and deliberately so: what moved is the strength of one supporting argument, not the chosen option, its drivers or its consequences. The accessibility finding was real, the lint was the blocker, and forward-only with the archive untouched is still the choice. Recorded here rather than silently edited, because the ratification attached to specific text and a reader is entitled to know which sentences changed after it. Note the irony and do not lose it: this record's own subject is a claim about the corpus asserted from impression rather than count, and its first draft made a smaller version of the same mistake.
