...
**Status:** proposed | **Oversight:** confirmed
**Chosen:** Chosen option: **adopt the upstream band shape, keep the threshold, keep the composition constraint**.
**Confirmation:** RISK-POLICY.md Label Bands read the adopted shape, the appetite line is; The gate's own parse was traced against the amended file by; docs/decisions/README.md carries an ADR-049 entry in the In-force section, and a; A pipeline assessment lands a residual of exactly 5 and prints it as Low with; Tom read and ratified this record in session on 2026-08-09.
**Related:** ADR-086, ADR-007, ADR-008, ADR-027
...
**Status:** accepted | **Oversight:** confirmed
**Chosen:** Chosen option: **"Every reviewer must return a pass"**, because the driver is not the single staleness defect but the multi-edition accumulation of deferrals, and options 2 and 3 both preserve a deferral path.
**Confirmation:** No edition publishes while any reviewer that scored it holds a finding that reviewer classified as a defect. T...; The reviews sibling records, per gate, the digest scored and whether each surviving finding is a defect or a d...; A deterministic pre-save check in scripts/check-newsletter-structure.sh, alongside check (m), asserts that eve...; SKILL.md describes this rule rather than the one it replaces: the preamble, step 15.37's exit condition, step ...; The residual-advisory section of a published edition's reviews file contains only findings the author explicit...
**Related:** ADR-043, ADR-047, ADR-015, ADR-020, ADR-042, ADR-044

### ADR-053 - The Shift adopts an H2-rooted section outline from Issue 17, and the archive is left alone
**Status:** accepted | **Oversight:** confirmed
**Chosen:** Chosen option: **"Adopt forward, leave the archive alone"**.
