...
Both jq scans then run, and a new pure helper `manifest_refresh_route <changed> <violations>` decides what happens next: `rollback` on violations, `commit` on a clean change, `skip` otherwise. It sits above the `PUSH_WATCH_LIB_ONLY` seam, shaped like the existing `deps_gate_route` that ADR-034's fail-fast routing uses, and is covered by new cases in `scripts/push-watch.test.mjs`.

The route REPLACES the existing `if ! git diff --quiet -- package.json package-lock.json` condition rather than sitting beside it. That is load-bearing: on the rollback path with an operator-dirty `package.json` the old condition is still true, so leaving it in place would commit a desynced pair and reintroduce the defect this RFC exists to remove. On `rollback` the script restores only the manifests it recorded clean beforehand (a file the operator had already modified is left alone, and the message says so), prints the violations, and falls through without committing and without exiting. Because it does not exit, the existing stale-deps gate twenty lines below fires and routes to `npm run fix:deps` exactly as it does today.

So push:watch either completes its own manifest write coherently and commits it, or undoes its own half-finished write and degrades to the behaviour it had before ADR-021. It adds no halt, adds no routing message, and never reverts a file it did not author in that same block. Only the two millisecond offline jq scans are added: no lint, no test, no build, because ADR-034 considered and rejected moving the fix flow into `push:watch` on direct user instruction, and a composite gate there would be that rejected option.
