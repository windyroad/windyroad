...
# Ask Hygiene 2026-05-30 (AFK work-problems iter 11, P061 KE to Verifying)
...
- P061 (assistant gates policy-authorised actions on user permission when risk-scorer has already cleared) Known Error to Verification Pending per ADR-022. Memory-layer fix shipped 2026-05-30 in commit `9f42130` (AFK iter 10 earlier today); ticket sat in `known-error/` awaiting transition. Release marker is the commit itself (`9f42130`), NOT a tagged npm release. The fix surface is project-local user memory at `~/.claude/projects/-Users-tomhoward-Projects-windyroad/memory/feedback_no_pitching_act_on_obvious_decisions.md`, which is not git-tracked and has no release pipeline. Same pattern as P062 (project-local `.claude/skills/wr-newsletter/` change in commit `de6c165`), P063 (project-local `.claude/skills/wr-newsletter-cover/` change in commit `eb0e8be`), and P066 (`docs/VOICE-AND-TONE.md` edit). Verified the memory file on disk before transitioning: "Wrap-up surface for policy-authorised post-clearance actions (P061)" section is present with the four canonical deferral-prose phrasings, the post-risk-scorer-clearance trigger condition, the equivalence rule, and the 2026-05-14 source incident citation. Commit-as-release-marker contract satisfied; KE to Verification Pending fires per ADR-022 user-memory carve-out.
- Mechanism: `git mv docs/problems/known-error/061-*.md docs/problems/verifying/061-*.md`; `Edit` tool updated Status field to "Verification Pending", added `**Released**:` line, inserted `## Fix Released` section between the Root Cause and Fix Strategy section and the Verification section (citing the release marker, fix mechanism, and same-pattern siblings); re-staged the `.verifying.md` path per P057 staging-trap discipline. README.md refresh per P062: P061 row removed from WSJF Rankings; P061 row inserted in Verification Queue immediately after P057 (both Released 2026-05-30, tiebreak by ID ASC per P150) and before P062; Notes section entry added matching the verifying-transition pattern of P010, P057+P066, P063. Line 3 Last-reviewed fragment per P134: rotated prior P068-Parked fragment to README-history.md under existing `## 2026-05-30` heading; wrote new P061-verifying fragment.
- Architect review (this iter): invoked on the retro-file append (docs/retros/ is NOT on the architect-gate exclusion list, unlike docs/problems/). Verdict: PASS. The append is structurally identical to the existing iter 6/7/8/9/10 sections already committed in the file. No new architectural decision implicated; the cited ADRs are upstream wr-itil / wr-architect / wr-risk-scorer plugin ADRs that downstream-consumer documentation legitimately references. The principal commit (`d01cbd0`, ticket transition) did NOT invoke architect because docs/problems/ IS on the exclusion list. Two-tier discipline: docs/problems/ excluded; docs/retros/ included.
- JTBD review (this iter): invoked on the retro-file append. Verdict: PASS. Pure operator backlog hygiene; no documented commercial-reader job is touched. The change does not modify any homepage, `/ai-quality`, `/founders`, `/vibe-code-audit`, `/blog`, or newsletter surface. No `// @jtbd` annotations apply (markdown retro files are not in the annotation surface).
- Risk-scorer pipeline (this iter, on the principal P061 transition commit): commit=1 (Very Low), push=0, release=0; cumulative cleared. RISK_BYPASS: reducing (advances P061 closure track from Known Error to Verification Pending per ADR-022; reducing-bypass criterion 1 satisfied per the prior transition-pattern precedent). Commit `d01cbd0` landed cleanly.
- Step 4a verification-close drain: P061 itself becomes a new Verification Queue row this iter, but per same-session exclusion (Step 4a sub-step 8 plus P282 sub-step 9c) a session cannot verify its own fix beyond "transition landed at commit time"; subsequent-session exercise is the meaningful signal. README Verification Queue's other `Likely verified?` cells remain `no (not observed)` or `no (observed regression)` across the row range; no `yes, observed:` rows surface for the P282 prior-session evidence drain on this bookkeeping-only iter.
- Step 2b pipeline-instability scan: ONE friction signal worth recording (not ticket-worthy this iter, recurrence-watch deferred). The no-em-dash hook tripped once on the first `Edit` attempt for the `## Fix Released` section (drafter used em-dash characters in connector and parenthetical phrasings). Hook blocked the write with the canonical message instructing the drafter to rewrite without em-dashes. Mitigation: rewrote the offending phrasings using semicolons, parentheses, and the preposition "to" for connector positions; second `Edit` landed cleanly. Recurring-pattern signal class: same hook caught the same drafter-pattern in earlier iters of this AFK run. Not ticket-worthy by itself (the hook does its job; drafter-side rewrite cost is bounded around one round-trip), but worth recurrence-watching: if it fires more than once per draft session, consider a prose-template that pre-substitutes em-dash-equivalents in the most common phrasings ("to" for connectors, ";" for clause separators, parens for parentheticals). Deferred as observation, not ticket.
- P057 staging-trap discipline held: pre-staged all three touched paths (`docs/problems/verifying/061-*.md`, `docs/problems/README.md`, `docs/problems/README-history.md`) together before the commit; landed in a single commit per ADR-014 grain. Combined-stage pattern from iters 8/9/10 extended cleanly to iter 11. The post-rename `git mv` plus `Edit` workflow correctly required an explicit `git add` of the new path (P057 trap mechanism), which the iter handled in-flow.
- Step 2b README inventory-currency advisory: not invoked (no `packages/` directory in this consumer project; fail-soft per ADR-013 Rule 6 as in prior iters).
- Step 2c context-usage: cheap layer measurement deferred this iter (script not load-bearing on the transition; observation-only iter without bytes-bucket drift signal). Prior iter (7 P042) recorded problems=557840B; this iter touches docs/problems/{verifying/, README.md, README-history.md} for a net delta of around 15 to 20 KB (README.md row shift, new VQ row, new Notes entry, line 3 replacement; README-history.md prior-fragment append). Within Tier-3 envelope.
- No deferrals to next interactive session from this iter. The P061 verification trigger (next AFK iter or session wrap-up where risk-scorer cleared every commit AND assistant pushes directly without any of the four canonical deferral-prose phrasings) is a behavioural-observation trigger fired by the operator on subsequent sessions, NOT a question requiring direction.

---

# Ask Hygiene 2026-05-30 (AFK work-problems iter 12, P060 capture-adr template em-dash)
...
- P060 (`capture-adr` SKILL.md template frontmatter sentinel carries U+2014 that no-em-dash hook blocks) Open to Parked, upstream-blocked. Fix site `packages/architect/skills/capture-adr/SKILL.md` does NOT exist in this project's working tree; the template literal lives in the `wr-architect` plugin at `~/.claude/plugins/cache/windyroad/wr-architect/<version>/skills/capture-adr/SKILL.md`, inside the `windyroad/agent-plugins` repo. This project is a downstream marketplace consumer of `@windyroad/wr-architect`. A consumer cannot edit the cached SKILL.md without losing the change on next plugin update.
- Verified 2026-05-30 on cached `0.12.1`: four U+2014 template literals still present at `skills/capture-adr/SKILL.md` lines 76 (frontmatter sentinel `decision-makers: [unspecified ... fill at canonical review]`), 96 (first considered-options `1. **Option A (chosen)** ... <summary>`), 97 (second considered-options `2. (deferred ... see /wr-architect:create-adr canonical review)`), and 129 (reassessment-criteria `(deferred to /wr-architect:create-adr canonical review ... default reassessment-date 3 months from capture)`). All four part of the template literal written verbatim into new `.proposed.md` ADR files via the capture-adr skill.
- Local whitelist option (extend `.claude/hooks/no-em-dash.sh` and `.claude/hooks/no-em-dash-bash.sh` WHITELIST_LINE entries with the capture-adr template sentinels) was considered but rejected: it is a band-aid that would orphan once upstream substitutes the U+2014 characters, and propagating it locally encodes whitelist debt across every adopter shipping the no-em-dash hook style. The local hook already carries three whitelist entries for upstream wr-itil contractual markers (P063 upstream-pending, capture-problem Priority + Effort lines); adding four more wr-architect entries is the wrong direction. Park-only is the consistent action with iters 3-9 (which made the same tradeoff for wr-architect P021 + P022, wr-itil P027 + P033 + P049, wr-jtbd P042, wr-risk-scorer P047).
- No upstream issue exists for P060 specifically. `gh issue list -R windyroad/agent-plugins --search "capture-adr em-dash"`, `--search "capture-adr unspecified canonical review"`, and `--search "wr-architect capture-adr template"` on 2026-05-30 all returned empty. Ticket carries the standing "Upstream report pending" note (whitelisted marker). Per AFK discipline iters 3-9: park-only, defer `/wr-itil:report-upstream` to a batched filing pass at a session boundary rather than firing it in-loop.
- Un-park trigger: a new `wr-architect` plugin release whose `skills/capture-adr/SKILL.md` template substitutes the four U+2014 characters with commas, colons, or parentheses. Canonical-expansion auto-detection string `(deferred to /wr-architect:create-adr canonical review)` is already long-dash-free per the ticket's Symptoms section, so the template substitution does not require changing the detection contract. Verify by re-reading the cached file in the new version, then capturing a new ADR via `/wr-architect:capture-adr` in this project; the first Write should succeed without the no-em-dash hook deny. Close P060 once a capture-adr invocation lands an ADR file on the first Write under the upgraded template.
- Composes-with iters 3 (P021), 4 (P022), 5 (P027), 6 (P033), 7 (P042), 8 (P047), 9 (P049): eight consecutive parks today sharing the marketplace-consumer-cannot-edit-cached-plugin pattern. P060 extends the wr-architect surface from hook scripts (P021, P022) to SKILL.md template literals (capture-adr). Same root cause class, same un-park mechanics (plugin release with the upstream-side substitution).
- Architect review (this iter): PASS on substance for the park-only decision (no ADR conflicts; no decision violations; the cited ADRs ADR-014 single-commit governance, ADR-022 verification lifecycle, ADR-049 PATH-shim, P057 staging-trap, P062 README-refresh, P094 README-refresh-on-create, P134 last-reviewed-line discipline are all in the well-trodden lane of prior iters today). The Open to Parked transition needs no new architectural call.
- JTBD review (this iter): PASS, no edits blocked. Pure operator backlog hygiene; no documented commercial-reader job touched. The change does not modify any homepage, `/ai-quality`, `/founders`, `/vibe-code-audit`, `/blog`, or newsletter surface. No `// @jtbd` annotations apply (markdown ticket + README files are not in the annotation surface).
- Risk-scorer pipeline (this iter, on the principal P060 park commit): commit=3 (Low), push=0, release=0; cumulative cleared within RISK-POLICY.md appetite. Bypass marker written by the PostToolUse:Agent hook after the wr-risk-scorer:pipeline subagent completed. Commit `a08d6dc` landed cleanly per ADR-014 single-commit grain.
- Step 4a verification-close drain: README Verification Queue's `Likely verified?` cells remain `no (not observed)` or `no (observed regression)` across the row range; no `yes - observed:` rows surface for the P282 prior-session evidence drain on this bookkeeping-only iter. P061 (verifying as of iter 11 today) is excluded by same-session rule; subsequent-session exercise is the meaningful signal.
- Step 2b pipeline-instability scan: P057 staging-trap discipline held cleanly. Pre-staged all four touched paths (`docs/problems/parked/060-*.md` added, `docs/problems/open/060-*.md` deleted via `git mv`, `docs/problems/README.md` modified, `docs/problems/README-history.md` modified) together via `git add -u docs/problems/` catch-all before invoking the principal commit. Single-commit landing verified post-commit (`a08d6dc`). The combined-stage discipline pattern from iters 8/9/10/11 extended cleanly to iter 12; no two-commit split.
- Step 2b README inventory-currency advisory: failed open (`packages/` directory absent in this consumer project) per ADR-013 Rule 6 fail-soft contract, as in every prior iter today. Not a regression.
- Step 2c context-usage cheap layer: problems=581165B (delta +23325B vs iter 7's 557840B snapshot), memory=369517B (delta 0), decisions=283436B (delta 0), skills=110533B (delta 0), jtbd=23200B (delta 0), hooks=14402B (delta 0), project-claude-md=7530B (delta 0); briefing not-measured (source-absent: project still uses legacy `docs/BRIEFING.md` single-file shape; P100 transition pending). Problems-bucket delta tracks the eight Open to Parked transitions today plus the P061 KE to V transition plus README-history rotations, plus the new P073 / P070 / P071 / P072 / P058 / P060 entries earlier today. All buckets within Tier-3 envelope.
- No deferrals to next interactive session from this iter. The P060 un-park trigger (new `wr-architect` plugin release with template U+2014 substitution) is a passive observation trigger fired by future cached-plugin reads, NOT a question requiring direction.
