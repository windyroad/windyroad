...
**Last reviewed:** 2026-06-17 (Tom-curated: banned list kept, corpus positives dropped, services-arm hyphenated stack banned, P077)
...
> Grounded in Tom's documented rejections recorded in problem ticket P077. Each entry is a phrasing the voice-tone gate passed on the abstract rules above but Tom rejected on editorial review. This is a settled rule set, not a draft.

This section gives the voice-tone gate a concrete oracle for "phrasings Tom rejects", separate from the abstract voice principles above, which pass copy that matches the rules but not Tom's voice.

Positive Tom-voice examples were deliberately NOT seeded from the published-edition corpus: those editions are AI-drafted then edited, so the surviving phrasing is mostly drafter-words Tom tolerated rather than authentic Tom-voice. A future positive seed should come from Tom-authored material; it is deferred until that material exists.
...
| Abstract-noun stacks | "substrate provider", "eval-governance reference structures", "containment patterns", "sandboxing patterns" (as a heading phrase), "services-arm" (the hyphenated stacked heading form) | Each matched the gloss-on-first-use rule mechanically but failed the CTO-from-a-non-AI-native-company clarity test (see Reading level and jargon load above). The open-prose "services arm" (e.g. "the vendor's services arm") stays acceptable; only the hyphenated stacked form is banned. |
